...
`opening-selector/wayfinder/` and `chess-classroom/wayfinder/` — its own map, its own flat
ticket numbering, scoped to the opening-tree effort only.

- `map*.md` — a map. Frontmatter: `labels: [wayfinder:map]`. More than one map can coexist
  when a new effort's destination is distinct from an existing map's — each is independent; a
  closed map is not reopened for a new destination.
- `tickets/NNNN-slug.md` — child tickets of a map, sharing one flat, globally-numbered folder
  across all maps in *this* wayfinder instance. Frontmatter:
  - `id`: NNNN
  - `title`
